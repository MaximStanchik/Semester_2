...
int HashFunction(int key, int size, int p) //для вычисления хэша ключа
{
	double key2 = 5 * ((0.6180339887499 * key) - int((0.6180339887499 * key)));
	return (p + key) % size;
}

int Next_hash(int hash, int size, int p) { //для решения коллизий при добавлении элементов в хэш-таблицу

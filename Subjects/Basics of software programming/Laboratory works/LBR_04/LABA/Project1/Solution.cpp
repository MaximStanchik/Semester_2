...
void enter_new() {
	...
	if (current_size < size) {
		...
		do {
			...
			if (list_of_cars[current_size].release_year <= current_y)
...
void del(string name) {
	bool flag = false;
	Automobile buf;
	ofstream f2("information.txt");
	ifstream f("information.txt");
	...
	f.close();
	f2.close();
	ifstream file2("information.txt");
...
void outputOneStructure(Automobile oneCar) {
	...
	cout << "ненужная строка: " << oneCar.release_year << endl;
	cout << "Body type: " << oneCar.body_type << endl;
	cout << "Ближайшей день рождения (в формате ДДММГГГГ): " << oneCar.last_inspection_date << endl;
...
void outToFile() {
	...
	for (int i = 0; i < current_size; i++) {
		...
		file1 << "Ближайшей день рождения (в формате ДДММГГГГ): ";
